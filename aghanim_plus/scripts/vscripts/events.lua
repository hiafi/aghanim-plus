...
--------------------------------------------------------------------------------
function CAghanim:LoadBlessingsHub(hHero, nLevel)
	return _G.BLESSING_LEVEL_HUB[nLevel + 1]
end
...
function CAghanim:InitBlessings( hHero )
	...
	if self:GetAscensionLevel() < AGHANIM_ASCENSION_GRAND_MAGUS then
		for i = 1, self:GetStartingCursedItemSlots() do
			local key = GrantItemDropToHero( hHero, "item_cursed_key" )
			if IsValid(key) then
				key:EndCooldown()
				key:CastAbility()
			end
	...
	if tBlessings == nil then
		print ( "Well, " .. hHero:GetUnitName().." has no blessings." )
		if GetMapName() == "hub" then
			local level = self:GetAscensionLevel()
			print ("However, this games starts in arcade mode at level:"..level)
			
			tBlessings = self:LoadBlessingsHub(hHero, level)
		else
			return
		end
	end
	...
	local players = self:GetConnectedPlayers()

	local blessing_amp = 1 + (AGHANIM_PLAYERS - #players) / (#players or 1) * _G.BLESSING_PLAYER_ABSENSE_BONUS
	-- local blessing_amp = 1 + (AGHANIM_PLAYERS - #players) * _G.BLESSING_PLAYER_ABSENSE_BONUS

	for szBlessingType,nBlessingValue in pairs( tBlessings ) do
		nBlessingValue = nBlessingValue * blessing_amp
...
function CAghanim:OnEntityKilled_PlayerHero( event )
	...
	self:SpecificHeroDeathHandle(killedHero)
	--  这里判断目标是否为重生
	local bImmortal = IsImmortalHero(killedHero)
	if bImmortal then 
		print("this hero is immortal")
	else
		print("this hero is NOT immortal")
	end
...
function CAghanim:SpecificHeroDeathHandle(hHeroKilled)
	local szHeroName = hHeroKilled:GetName()
	
	local death_handle = _G.HERO_DEATH_HANDLES[szHeroName]

	if death_handle ~= nil then
		death_handle(hHeroKilled)
	end
end

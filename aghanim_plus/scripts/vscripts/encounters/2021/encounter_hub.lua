...
function CMapEncounter_Hub:CustomStartupCompleted()

	self:SpawnBossPreviewEntity()

	local vecPlayers = CAghanim:GetConnectedPlayers()
	local bExtraLegendary = #vecPlayers < 3
	if bExtraLegendary and self.hRoom:GetDepth() < 3 then	
		local szRoomDepth = "20"
		...
		local RewardOptions = {}
		for _, nPlayerID in pairs(vecPlayers) do
			if PlayerResource:IsValidPlayerID(nPlayerID) then
				local vecPlayerRewards = GetRoomRewards( tonumber(szRoomDepth), false, nPlayerID ) 
				RewardOptions[ tostring(nPlayerID) ] = vecPlayerRewards;
			end
		end
		RewardOptions[ "rarity" ] = "epic"

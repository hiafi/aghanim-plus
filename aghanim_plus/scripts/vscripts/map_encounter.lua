...
function CMapEncounter:DropItemFromRoomRewardContainer( hContainer, szItemName, bAnnounce, hPlayerHero )
	for szNeutralItem,v in pairs ( PRICED_ITEM_REWARD_LIST ) do
		if szNeutralItem == szItemName then
			local level = PRICED_ITEM_REWARD_LEVEL_LIST[szItemName]
			local szRealItemName = szItemName;
			if math.random() > 0.5 then
				szRealItemName = "item_tier"..level.."_token";
			end
			DropNeutralItemAtPositionForHero( szRealItemName, hContainer:GetAbsOrigin() + RandomVector( RandomFloat( 50, 150 ) ), ( hPlayerHero ~= nil and hPlayerHero ) or PlayerResource:GetSelectedHeroEntity( 0 ), -1, true )
			return
		end
	end

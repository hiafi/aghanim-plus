...
function CAghanim:InitScoreboardInfo()
	...
	self.ScoresLastUpdateTime = Time()
	self.playerDamageTable = {}

	for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
		self.playerDamageTable[nPlayerID] = 0
		local kv = 
		{
			kills = 0,
			death_count = 0,
...
function CAghanim:SetAscensionLevel( nLevel )
	local bActualSettled = true;
	if nLevel < 0 then
		nLevel = 0
		bActualSettled = false
	...
	self.bHasSetAscensionLevel = bActualSettled
...
function CAghanim:ApplyGlobalAscensionModifiers()
	if self.nAscensionLevel >= AGHANIM_ASCENSION_MAGICIAN then 
		self.nStartingLives = AGHANIM_STARTING_LIVES - ASCENSION_MAGICIAN_LESS_STARTING_LIVES
...
function CAghanim:UpdatePlayersScoringState()
	if Time() - self.ScoresLastUpdateTime < 3.0 then
		return
	end
	-- currently only health
	for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
		local kv = CustomNetTables:GetTableValue( "aghanim_scores", tostring(nPlayerID) )
		if kv ~= nil and PlayerResource:IsValidPlayerID(nPlayerID) then
			kv.heal = PlayerResource:GetHealing(nPlayerID)
			kv.damage = self.playerDamageTable[nPlayerID]
			CustomNetTables:SetTableValue( "aghanim_scores", tostring(nPlayerID), kv )
		end
	end
end
...
function CAghanim:OnThink()
	...
	elseif nGameState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		...
		self:UpdatePlayersScoringState()

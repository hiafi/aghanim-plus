...
function modifier_generic_ring_lua:SetCallback( callback )
	...
	self.start_time = self:GetElapsedTime()
...
function modifier_generic_ring_lua:OnIntervalThink()
	local radius = self.start_radius + self.speed * (self:GetElapsedTime() - self.start_time)
	...
	for _,target in pairs(targets) do
		...
		if not self.targets[target] then
			...
			if (self.IsCircle == 0) or (target:GetOrigin()-self:GetParent():GetOrigin()):Length2D()>(radius-self.width) then
